...
#if 0
static void
selected_context_changed (GtkListStore *list, IconBrowserWindow *win)
{
  GtkWidget *row;
  GtkWidget *label;
...
int main(int argc, char *argv[]) {
  ...
  //  Context *current_context;
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Stock Icon Browser");
    ...
    //gtk_icon_view_enable_model_drag_source (GTK_ICON_VIEW (list),
      //                                      GDK_BUTTON1_MASK,
        //                                    targets, n_targets,
          //                                  GDK_ACTION_COPY);
    ...
    setup_image_dnd (image1);
    setup_image_dnd (image2);
    setup_image_dnd (image3);
    setup_image_dnd (image4);
    setup_image_dnd (image5);
    
    //win->contexts = g_hash_table_new_full (g_str_hash, g_str_equal, NULL, g_free);
    
    gtk_tree_model_filter_set_visible_func (filter_model, icon_visible_func, window, NULL);
    gtk_window_set_transient_for (GTK_WINDOW (details), GTK_WINDOW (window));
